...
void setup(void){
    ...
    UCSR0C |= (1<<USCZ01) | (1<<UCSZ00); // znak 8 bitowy, 1 bit stopu, bez parzystosci : 8n1
    UCSR0B |= (1<<TXEN0) // wlaczony lacznik nadajnika, maska nadawcza zdjeta //zakomentowanie spowoduje brak nadawnia
    //USCSR0B |= (1<<TXCIE0); //maska nadawcza zdjeta
    UCSR0B |= (1<<RXEN0); //wlaczony odbiornik
    //UCSR0B |= (1<<RXCIE0); //maska odbiorcza zdjeta
    //sei();
    setup_wyswietlacz();
}
...
void loop(void){
    ...
    //gorsza bo zuĹĽywa proces procesora; nalezy odciÄ…ĹĽaÄ‡ pracÄ™ procesora przy obserwacji peryferiĂłw
    //kaĹĽde zdarzenia ma swojÄ… flagÄ™ w wektorze przerwaĹ„.
    //zastapienie przerwania isr(isart_TX_vect); czy to prawda, ze flaga jest niezerowa; czy ustawila sie 
    if(UCSR0A & (1<<TXC0))
    {
        //nadaj();
    }
